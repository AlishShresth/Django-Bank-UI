...
export function DepositWithdrawalForm({ accounts, onTransaction }: DepositWithdrawalFormProps) {
  ...
  if (!user || (!hasPermission(user.role, "process_deposits") && !hasPermission(user.role, "process_withdrawals"))) {
    return null
  }
  ...
  const validateForm = (data: any, type: "deposit" | "withdrawal") => {
    ...
    if (!data.amount) {
      ...
      if (account && Number(data.amount) > account.balance) {
  ...
  const handleSubmit = async (e: React.FormEvent, type: "deposit" | "withdrawal") => {
    ...
    try {
      ...
      // Reset form
      if (type === "deposit") {
        setDepositData({ accountId: "", amount: "", description: "", reference: "" })
      } else {
        setWithdrawalData({ accountId: "", amount: "", description: "", reference: "" })
      }
      setErrors({})
    } catch (error: any) {
      setErrors({ general: error.message || `${type} failed. Please try again.` })
    } finally {
      setIsLoading(false)
    }
  }
            ...
            <form onSubmit={(e) => handleSubmit(e, "deposit")} className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(value) => setDepositData({ ...depositData, accountId: value })}
                >
                  <SelectTrigger>
            ...
            <form onSubmit={(e) => handleSubmit(e, "withdrawal")} className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(value) => setWithdrawalData({ ...withdrawalData, accountId: value })}
                >
                  <SelectTrigger>
                    <SelectValue placeholder="Select account" />
                  </SelectTrigger>
              ...
              <FormInput
                ...
                onChange={(e) => setWithdrawalData({ ...withdrawalData, reference: e.target.value })}
