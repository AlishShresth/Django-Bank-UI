...
import { MoreHorizontal, Eye, EyeOff, Shield, AlertTriangle, CreditCard } from "lucide-react"
...
export function CardDisplay({ card, onBlock, onUnblock, onReportLoss, onRequestReplacement }: CardDisplayProps) {
  ...
  const canManageCard = user && hasPermission(user.role, "manage_cards")

  return (
    <div className="space-y-4">
      {/* Card Visual */}
      <div
        className={`relative w-full h-56 rounded-xl bg-gradient-to-br ${getCardTypeColor(card.cardType)} p-6 text-white shadow-lg`}
      >
        <div className="flex justify-between items-start">
          <div>
            <p className="text-sm opacity-80">SecureBank</p>
            <p className="text-xs opacity-60 uppercase tracking-wider">{card.cardType} Card</p>
          </div>
          ...
          <div className="flex justify-between items-end">
            ...
            {card.balance !== undefined && (
              <div className="text-right">
                ...
                <p className="text-lg font-semibold">{formatBalance(card.balance)}</p>
              </div>
            )}
            {card.creditLimit && (
              <div className="text-right">
                <p className="text-xs opacity-60">CREDIT LIMIT</p>
                <p className="text-lg font-semibold">{formatBalance(card.creditLimit)}</p>
              </div>
            )}
          </div>
        </div>
        ...
        <CardContent className="p-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-muted-foreground">Manage your card settings and security</p>
            </div>
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end">
                {card.status === "active" && onBlock && (
                  <DropdownMenuItem onClick={() => onBlock(card.id)} className="text-warning">
                    <AlertTriangle className="h-4 w-4 mr-2" />
                    Block Card
                  </DropdownMenuItem>
                )}
                {card.status === "blocked" && onUnblock && (
                  <DropdownMenuItem onClick={() => onUnblock(card.id)} className="text-success">
                    <Shield className="h-4 w-4 mr-2" />
                    Unblock Card
                  </DropdownMenuItem>
                )}
                {onReportLoss && (
                  <DropdownMenuItem onClick={() => onReportLoss(card.id)} className="text-destructive">
                    <AlertTriangle className="h-4 w-4 mr-2" />
                    Report Lost/Stolen
                  </DropdownMenuItem>
                )}
                {onRequestReplacement && (
                  <DropdownMenuItem onClick={() => onRequestReplacement(card.id)}>
